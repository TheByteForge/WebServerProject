...
public class Resource {
    ...
    public String absolutePath(){
        ...
        if(this.isScriptAliased()){
            resolvedPath = resourceURI.replaceFirst(tempLine[2], "").replaceAll("/", "") + resolvedString.replaceAll("\\\\", "/");
            String givenPath = resourceURI +tempLine[2];
            System.out.print(resolvedPath);
        }
        else if(this.isAliased()){
            resolvedPath = resourceURI.replace(tempLine[2], "").replaceAll("/", "") + resolvedString.replaceAll("\\\\", "/");
            String givenPath = resourceURI +tempLine[2];
            System.out.print(resolvedPath);

        }
        //I THINK DIRECTORY INDEX HERE
//        else if(){
//
//        }
